...
namespace BOSERP.Modules.MEEmr
{
    ...
    {
        public MEEmrDocumentsGridControl()
        {
        ...
        {
            Console.WriteLine("protected override void GridView_RowClick(object sender, EventArgs e)");
            base.GridView_RowClick(sender, e);
            GridView gridView = (GridView)this.MainView;
            GridHitInfo hi = gridView.CalcHitInfo(this.PointToClient(MousePosition));
            var topIdx = gridView.TopRowIndex;
            if (hi.InGroupRow || gridView.IsGroupRow(hi.RowHandle))
            {
                gridView.FocusedRowHandle = GetFocusedRowHandleByCurrentDocument();
                gridView.TopRowIndex = topIdx;
                return;
            ...
            int currentFocus = gridView.FocusedRowHandle;
            if (gridView.FocusedRowHandle >= 0 && module.IsNothingToSaveDocumentContent())
            {
                var doc = gridView.GetRow(gridView.FocusedRowHandle) as MEEmrDocumentsInfo;
                module.ReloadIfEmrChangedByOther(doc);
                gridView.FocusedRowHandle = currentFocus;
                doc = gridView.GetRow(gridView.FocusedRowHandle) as MEEmrDocumentsInfo;
                module.InvalidateDocument(doc, asyncMode: true);
                module.RunActionsOnOpen(doc);
            }
            else
            {
                var rowIdx = GetFocusedRowHandleByCurrentDocument();
                gridView.FocusedRowHandle = rowIdx;
            }
        }
