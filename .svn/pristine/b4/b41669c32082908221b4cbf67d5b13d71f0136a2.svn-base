...
namespace BOSERP.Utilities
{
	public class Ean13
	{
        ...
        private float _fHeight = 13f;
        private float _fFontSize = 8.0f;
        private float _fScale = 1.2f;
        ...
        public Ean13(string productBarcode)
        {
		...
		{
            if (!string.IsNullOrEmpty(this.ProductBarcode) && this.ProductBarcode.Length == 13)
            {
                ...
                sLeftPattern = ConvertLeftPattern(this.ProductBarcode.Substring(0, 7));
                ...
                strbEAN13.AppendFormat("{0}{1}{2}{3}{4}{1}{0}",
                    ...
                    sLeftPattern,
                    this._sSeparator,
                    ConvertToDigitPatterns(this.ProductBarcode.Substring(7), this._aRight));
                ...
                xPosition = xStart - g.MeasureString(this.ProductBarcode.Substring(0, 1), font).Width;
                float yPosition = yStart + (height - fTextHeight);

                // Draw 1st digit of the country code.
                g.DrawString(this.ProductBarcode.Substring(0, 1), font, brush, new System.Drawing.PointF(xPosition, yPosition));

                xPosition += (g.MeasureString(this.ProductBarcode.Substring(0, 1), font).Width + 43 * lineWidth) -
                    (g.MeasureString(this.ProductBarcode.Substring(1, 6), font).Width);

                // Draw MFG Number.
                g.DrawString(this.ProductBarcode.Substring(1, 6), font, brush, new System.Drawing.PointF(xPosition, yPosition));

                xPosition += g.MeasureString(this.ProductBarcode.Substring(1, 6), font).Width + (11 * lineWidth);

                // Draw Product ID.
                g.DrawString(this.ProductBarcode.Substring(7), font, brush, new System.Drawing.PointF(xPosition, yPosition));
		...
		{            
            ...
            g.FillRectangle(new System.Drawing.SolidBrush(System.Drawing.Color.White),
                new Rectangle(0, 0, (int)tempWidth, (int)tempHeight));
			this.DrawEan13Barcode( g, new System.Drawing.Point( 0, 0 ) );
			g.Dispose( );
			return bmp;
		}
        public System.Drawing.Graphics CreateGraphics(Image img)
        {
            float tempWidth = (this.Width * this.Scale) * 100;
            float tempHeight = (this.Height * this.Scale) * 100;
        ...
        public string CalculateChecksumDigit(string barcode12Digit)
		{			
			int iSum = 0;
			int iDigit = 0;

			// Calculate the checksum digit here.
            for (int i = barcode12Digit.Length; i >= 1; i--)
			{
                iDigit = Convert.ToInt32(barcode12Digit.Substring(i - 1, 1));
				if( i % 2 == 0 )
				{	// odd
					iSum += iDigit * 3;
				}
