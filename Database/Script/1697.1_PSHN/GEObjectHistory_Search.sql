...
BEGIN
	...
	SET NOCOUNT ON;
	DECLARE @stObjectNumber nvarchar(80) = ''
	DECLARE @stAction nvarchar(80) = ''
	IF (@GEObjectHistoryObjectNumber != '')
	BEGIN
		SET @stObjectNumber = 'AND GEObjectHistoryObjectNumber = @GEObjectHistoryObjectNumber'
	END

	IF (@GEObjectHistoryAction != '')
	BEGIN
		SET @stObjectNumber = 'AND GEObjectHistoryAction = @GEObjectHistoryAction'
	END
	...
	SET @sql = '
			WITH query AS (
				...
				WHERE [AAStatus]=''Alive'' AND GEObjectHistoryObjectName = @GEObjectHistoryObjectName
					AND (CONVERT(date,GEObjectHistoryDate) BETWEEN @GEObjectHistoryDateFrom AND @GEObjectHistoryDateTo)
					' + @stObjectNumber + @stAction +
					')
