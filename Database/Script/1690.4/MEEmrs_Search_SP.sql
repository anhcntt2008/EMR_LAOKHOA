GO
/****** 
	Object:  StoredProcedure [dbo].[MEEmrs_Search]    Script Date: 8/3/2020 3:12:22 PM 
	DateOut no use between. Wrong convert SqlValue # value in code.
******/
SET ANSI_NULLS ON
GO
...
SET NOCOUNT ON
	...
	WHERE  (@MEEmrNo IS NULL OR e.MEEmrNo LIKE ''%'' + @MEEmrNo + ''%'')
		...
		AND (CONVERT(date,e.MEEmrDateOut) >= @MEEmrDateOutFrom AND CONVERT(date,e.MEEmrDateOut) < @MEEmrDateOutTo)
