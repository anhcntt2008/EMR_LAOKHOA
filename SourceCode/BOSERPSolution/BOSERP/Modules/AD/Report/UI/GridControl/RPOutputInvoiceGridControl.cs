...
namespace BOSERP.Modules.Report
{
    public partial class RPOutputInvoiceGridControl : ReportGridControl
    {
        ...
        protected override GridView InitializeGridView()
        {
            ...
            GridColumn column = gridView.Columns["ACDocumentSample"];
            if (column != null)
            {
                column.OptionsColumn.AllowEdit = true;
            }
            column = gridView.Columns["ACDocumentNotation"];
            if (column != null)
            {
                column.OptionsColumn.AllowEdit = true;
            }
            column = gridView.Columns["ACDocumentInvoiceNum"];
            if (column != null)
            {
                column.OptionsColumn.AllowEdit = true;
            }
        ...
        //    ADConfigValuesController controller = new ADConfigValuesController();
        //    DataSet ds = controller.GetADConfigValuesByGroup(ConfigValueGroup.InvoiceInvoiceType);
        //    if (ds != null && ds.Tables.Count != 0)
        //    {
        //        foreach (DataRow row in ds.Tables[0].Rows)
        //        {
