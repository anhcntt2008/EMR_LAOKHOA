...
{
    public partial class guiUpdateEmrTemplate : BOSERPScreen
    {
        ...
        public List<MEEmrDocumentsInfo> _documents { get; private set; }
        public List<METemplateIndexsInfo> MappingList { get; private set; }

        public guiUpdateEmrTemplate(int typeID, List<MEEmrDocumentsInfo> documents)
        {
            InitializeComponent();
            _typeID = typeID;
        ...
        private void UpdateTemplate_Load(object sender, EventArgs e)
        {
            ...
            {
                if (string.IsNullOrEmpty(document.MEEmrDocumentGroup)) continue;
        ...
        private void GridView_RowStyle(object sender, RowStyleEventArgs e)
        {
            ...
            if (e.RowHandle >= 0)
            {
                string colIdNew = View.GetRowCellDisplayText(e.RowHandle, View.Columns["METemplateIndexIDNew"]);
                if (!string.IsNullOrEmpty(colIdNew))
                {
        ...
        {
            GridView View = sender as GridView;
            if (e.Column.FieldName == "METemplateIndexIDNew" && string.IsNullOrEmpty(e.CellValue.ToString()))
            {
