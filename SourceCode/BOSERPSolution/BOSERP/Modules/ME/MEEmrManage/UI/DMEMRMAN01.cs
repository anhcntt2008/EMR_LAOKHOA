...
namespace BOSERP.Modules.MEEmrManage.UI
{
    ...
    /// </summary>
    public partial class DMEMRMAN01 : BOSERPScreen
    {
        public DMEMRMAN01()
        {
            InitializeComponent();
        ...
        public override void InitializeScreen(STScreensInfo objStScreensInfo)
        {
            ...
            fld_dteSearchFromMEEmrDateOut.EditValue = DateTime.MaxValue;
            fld_dteSearchToMEEmrDateOut.EditValue = DateTime.MaxValue;
        }
        ...
        private void ChangeControlsState(bool enable)
        {
            ...
            fld_txtMEEmrNo.Enabled = enable;
            fld_lkeFK_MEEmrTypeID.Enabled = enable;
            chkDepartmentShared.Enabled = enable;
            fld_txtMEPatientNo.Enabled = !enable;
            fld_lkeFK_MEPatientID.Enabled = !enable;
            fld_lkeMEEmrPatientGroup.Enabled = enable;
            fld_dteSearchFromMEEmrDateOut.Enabled = enable;
            fld_dteSearchToMEEmrDateOut.Enabled = enable;
            fld_lkeMEEmrStatus.Enabled = enable;
        }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            (this.Module as MEEmrManageModule).Search(fld_ccbeFK_HRDepartmentID.EditValue.ToString(),
               ...
               fld_txtMEEmrNo.Text,
               fld_lkeFK_MEEmrTypeID.EditValue,
               fld_lkeMEEmrPatientGroup.EditValue.ToString(),
               ((DateTime)fld_dteSearchFromMEEmrDateOut.EditValue).Year == DateTime.MaxValue.Year
                   ? new DateTime(2005, 1, 1, 0, 0, 0)
                   : ((DateTime)fld_dteSearchFromMEEmrDateOut.EditValue).Date,
               ((DateTime)fld_dteSearchToMEEmrDateOut.EditValue).Year == DateTime.MaxValue.Year
                   ? DateTime.MaxValue
                   : ((DateTime)fld_dteSearchToMEEmrDateOut.EditValue).Date.AddDays(1).AddMilliseconds(-1),
               fld_lkeMEEmrStatus.EditValue.ToString() != null ? (fld_lkeMEEmrStatus.EditValue.ToString() == "New" ? "" : fld_lkeMEEmrStatus.EditValue.ToString()) : "");
        ...
        private void SearchByPatient()
        {
            ...
            {
                (this.Module as MEEmrManageModule).SearchByPatient(fld_lkeFK_MEPatientID.EditValue);
                return;
            }
        ...
        private void btnGroupArchiveEmr_Click(object sender, EventArgs e)
        {
            (this.Module as MEEmrManageModule).GroupArchiveEmrs();
        }
    }
}
